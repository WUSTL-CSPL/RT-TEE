...
#include "thread_private_core_kernel.h"
...
static void lock_single_instance(void)
{
	...
	if (tee_ta_single_instance_thread != thread_get_id()) {
		...
		while (tee_ta_single_instance_thread != THREAD_ID_INVALID){
			// IMSG("lock_single_instance wait signal");
...
static bool tee_ta_try_set_busy(struct tee_ta_ctx *ctx)
{
	...
	if (ctx->flags & TA_FLAG_CONCURRENT)
		return true;
	// mask native intr to ensure the finish of mutex_lock and mutex_unlock pare
	
	int can_lock_write = false;
	while(!can_lock_write){
		can_lock_write = mutex_trylock(&tee_ta_mutex);
	}
	...
	// unmask native intr to ensure the finish of mutex_lock and mutex_unlock pare
			...
			struct tee_ta_session_head *open_sessions)
{
	...
	TAILQ_FOREACH(s, open_sessions, link) {
		if ((vaddr_t)s == id) // session_id is here, need to find out what is it to set optee_msg_arg arg->session.
			// id is actually the virtual address to a tee_ta_session struct
			return s;
	}
	return NULL;
}
			...
			struct tee_ta_session_head *open_sessions)
{
	...
	while (true) {
		...
		//TODO this should be fixed when multi core 
		//reset here
		s->lock_thread = -1;
		//reset end
		if (!s)
			       ...
			       struct tee_ta_param *param)
{
	...
	if (!check_params(s, param)){
		IMSG("rd entered optee_os/core/kernel/tee_ta_manager.c:tee_ta_open_session, check_params failed...\n");
...
// 	IMSG("correlation task %d, thread %d", l->curr_task, secure_tasks[l->curr_task].curr_thread);
// }
...
TEE_Result tee_ta_invoke_command(TEE_ErrorOrigin *err,
				 ...
				 struct tee_ta_param *param)
{
	//IMSG("tee_ta_invoke_command");
	TEE_Result res;
	

	if (check_client(sess, clnt_id) != TEE_SUCCESS){
		IMSG("rd says check_client fails...\n");
		return TEE_ERROR_BAD_PARAMETERS; /* intentional generic error */
	}


	if (!check_params(sess, param)){
		IMSG("rd says check_params fails...\n");
		return TEE_ERROR_BAD_PARAMETERS;
	}
	...
	//IMSG("Now enter_invoke_cmd in tee_ta_manager.c!\n");
...
struct tee_ta_session *tee_ta_pop_current_session(void)
{
	...
	// IMSG("tee ta pop current session : busy =%d, ta session pointer=%x\n", tsd->ctx->busy, s);
	//IMSG("tee ta pop current session : busy =%x, ta session pointer=%x\n", tsd, s);
