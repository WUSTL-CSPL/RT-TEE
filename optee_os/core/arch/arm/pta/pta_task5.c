...
#define TA_NAME		"pta_task5.pta"

int task5_execution_time = 0;
			...
			TEE_Param params[TEE_NUM_PARAMS] __unused)
{
	...
	mult_busy_loop(task5_execution_time); 
	test_finish[get_current_core()][4] += 1; 
	return TEE_SUCCESS;
}
			...
			TEE_Param params[TEE_NUM_PARAMS])
{
	...
	int *shared_mem = params[0].memref.buffer;
	IMSG("shared mem content %d", *shared_mem);
	*shared_mem = 200;
		...
		void **ppSessionContext __unused)
{
	...
	task5_execution_time = curr_task_params->execution_time;
...
pseudo_ta_register(.uuid = PTA_TASK5_UUID, .name = TA_NAME,
