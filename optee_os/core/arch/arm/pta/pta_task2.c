...
#define TA_NAME		"pta_task2.pta"


int task2_execution_time = 0;
extern long loop_result;
extern long mult_busy_loop(int execution_time);
			...
			TEE_Param params[TEE_NUM_PARAMS] __unused)
{
	...
	mult_busy_loop(task2_execution_time); 
	test_finish[get_current_core()][1] += 1; 
	return TEE_SUCCESS;
}
		...
		void **ppSessionContext __unused)
{
	...
	task2_execution_time = curr_task_params->execution_time;
...
pseudo_ta_register(.uuid = PTA_TASK2_UUID, .name = TA_NAME,
