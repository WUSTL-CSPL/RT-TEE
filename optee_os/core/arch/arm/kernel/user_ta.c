...
//TODO this should be replaced by destroy task in close session
extern int init_scheduler;
extern int ta_open_session_flag[CFG_TEE_CORE_NB_CORE];
extern int get_current_core();


void init_dta_task_params(uintptr_t param_ptr){
	...
	task_params = (struct secure_task_params *)param_ptr;
			...
			const struct tee_ta_param *p, void *va[TEE_NUM_PARAMS])
{
	...
	for (n = 0; n < TEE_NUM_PARAMS; n++) {
		...
		case TEE_PARAM_TYPE_MEMREF_INOUT:
			a = (uintptr_t)va[n];
			b = p->u[n].mem.size;
			//the first param is used to set the task parameter for rt-tee scheduler
			if(ta_open_session_flag[get_current_core()] && n == 0){
				init_dta_task_params(a);
			}			
			break;
...
extern trigger_timer_micro_s(uint64_t time_micro_s);
