...
/*original tmp stack size is 2048*/
#ifdef ARM64
		...
		struct thread_smc_args *args)
{
	 ...
	 * Asynchronous abort and unmasked native interrupts.
	 */
	// IMSG("Before init %d", (thread->regs.cpsr >> 6) & THREAD_EXCP_ALL);

	//original codes
	// thread->regs.cpsr = SPSR_64(SPSR_64_MODE_EL1, SPSR_64_MODE_SP_EL0,
	// 			THREAD_EXCP_FOREIGN_INTR | DAIFBIT_ABT);

	//make all interrupt codes
	thread->regs.cpsr = SPSR_64(SPSR_64_MODE_EL1, SPSR_64_MODE_SP_EL0,
				THREAD_EXCP_FOREIGN_INTR | DAIFBIT_ABT | THREAD_EXCP_NATIVE_INTR);

	/* Reinitialize stack pointer */
	thread->regs.sp = thread->stack_va_end;
	// IMSG("After init %d", (thread->regs.cpsr >> 6) & THREAD_EXCP_ALL);
	/*
	 * Copy arguments into context. This will make the
...
// extern struct secure_task_tq_head runnable_tasks[SECURE_WORLD_MIN_PRIORITY];
extern int total_cmpl_tasks[CFG_TEE_CORE_NB_CORE];
extern int wait_to_execute[CFG_TEE_CORE_NB_CORE];
extern s_time_t getCurrentTime_micro();
extern s_time_t get_next_job_release(); 
extern int miss_index[5];
extern int get_current_core();
extern void return_to_normal_world();
extern secure_schedule();
extern rd_scheduler_stub();
extern uint64_t fiqORstd[CFG_TEE_CORE_NB_CORE];
extern int test_finish[CFG_TEE_CORE_NB_CORE][5];
extern void rt_tee_sched_item();


// FIXME: rd changed thread_alloc_and_run from static void to void and expose it in thread.h
void thread_alloc_and_run(struct thread_smc_args *args)
{
	...
	l->curr_thread = n;
	threads[n].flags = 0;
	init_regs(threads + n, args); // init_regs set the following

	/* Save Hypervisor Client ID */
	threads[n].hyp_clnt_id = args->a7;

	if(!fiqORstd[get_current_core()]){
		thread_lazy_save_ns_vfp();
		threads[n].vfp_state.ns_saved = true;
	}
...
extern int start_time[CFG_TEE_CORE_NB_CORE];


int finish_secure_task_from_scheduler(){


	if(fiqORstd[get_current_core()] == 0){
		return &fiqORstd[get_current_core()];
	}

	struct thread_core_local *l = thread_get_core_local();

	int curr_task = l->curr_task;
	test_finish[get_current_core()][curr_task] += 1;

	//remove current task from secure tasks runnable queue
	secure_tasks[curr_task].task_state = SECURE_TASK_FINISHED;
	secure_tasks[curr_task].curr_thread = -1;
	secure_tasks[curr_task].processing_core = -1;
	TAILQ_REMOVE(&runnable_tasks[get_current_core()][secure_tasks[curr_task].priority],\
	 &secure_tasks[curr_task], tail_queue_entity);

	//this is used for experiment
	total_cmpl_tasks[get_current_core()] += 1;

	secure_tasks[curr_task].lastCompletionOfJob = getCurrentTime_micro();
	//this statistic is used to judge whether a job should be executed
	secure_tasks[curr_task].numOfJobCompleted += 1;
...
// #ifdef RT-TEE_SYNTHETIC	
	if(secure_tasks[curr_task].lastCompletionOfJob > \
		secure_tasks[curr_task].lastReleaseOfJob + secure_tasks[curr_task].period){
		
		if(secure_tasks[curr_task].numOfJobReleased <= NUM_OF_JOBS){
			//experiment need,only measuring missed jobs in NUM_OF_MEASURED jobs
			if (secure_tasks[curr_task].numOfJobCompleted <= NUM_OF_MEASURED)
			{
				if(secure_tasks[curr_task].numOfJobDlMissed < NUM_OF_MEASURED){
					secure_tasks[curr_task].numOfJobDlMissed++;
					miss_index[curr_task] += 1;
				}
			}
		}
	}
...
void __weak __thread_std_smc_entry(struct thread_smc_args *args)
{
	...
	thread_std_smc_handler_ptr(args); // 	thread_std_smc_handler_ptr = handlers->std_smc;
...
int thread_state_suspend(uint32_t flags, uint32_t cpsr, vaddr_t pc)
{
	...
	// JW: What is this? What does it do?
...
{
	/* thread_get_core_local() requires foreign interrupts to be disabled */
	uint32_t exceptions = thread_mask_exceptions(THREAD_EXCP_NATIVE_INTR);
	struct thread_core_local *l;
	...
	if (enable) {
		...
		thread_set_exceptions(exceptions & ~THREAD_EXCP_NATIVE_INTR);
	} else {
		/*
		 * No need to disable native interrupts here since they're
		 * already disabled above.
		 */
		threads[l->curr_thread].flags &=
					~THREAD_EXCP_NATIVE_INTR;
...
static bool get_spsr(bool is_32bit, unsigned long entry_func, uint32_t *spsr)
{
	...
	if (is_32bit) {
		...
		*/
		...
		//rt-tee change 1
		s = read_daif() & (0x6 << SPSR_32_AIF_SHIFT);;
		// s = read_daif();
		// IMSG("daif %x\n", s);
...
extern void correlate_task_thread();
		...
		uint32_t *exit_status0, uint32_t *exit_status1)
{
	...
	uint32_t result = __thread_enter_user_mode(a0, a1, a2, a3, user_sp, entry_func,
...
extern int resume_num[CFG_TEE_CORE_NB_CORE];
extern int suspend_num[CFG_TEE_CORE_NB_CORE];


void rt_tee_thread_state_suspend(uint32_t cpsr){

	fiqORstd[get_current_core()] = 0;
	struct thread_core_local *l = thread_get_core_local();
	int ct = l->curr_thread;
	...
	if (is_from_user(cpsr)) {
		...
		fp_regs_save(&secure_tasks[l->curr_task].saved_fp_regs);
	...
	l->curr_thread = -1;
	if(secure_tasks[l->curr_task].task_state != SECURE_TASK_RUNNING){
		IMSG("not finish invoke");
		threads[ct].state = THREAD_STATE_FREE;
	}
	...
	unlock_global();
	suspend_num[get_current_core()] += 1;
}



void rt_tee_thread_state_active(int to_active_task){
	...
	//add new
	int to_active_thread = secure_tasks[to_active_task].curr_thread;

	assert(threads[to_active_thread].state == THREAD_STATE_SUSPENDED);


	threads[to_active_thread].state = THREAD_STATE_ACTIVE;

	l->curr_thread = to_active_thread;
	l->curr_task = to_active_task;


	if (is_user_mode(&threads[to_active_thread].regs))
		tee_ta_update_session_utime_resume();

	if (threads[to_active_thread].have_user_map)
		core_mmu_set_user_map(&threads[to_active_thread].user_map);
	...
	//restore fp_regs for dta
	fp_regs_restore(&secure_tasks[to_active_task].saved_fp_regs);

	
	unlock_global();

	resume_num[get_current_core()] += 1;
