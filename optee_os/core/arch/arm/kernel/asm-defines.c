...
DEFINES
{
	...
	DEFINE(THREAD_CORE_LOCAL_CURR_TASK,
		offsetof(struct thread_core_local, curr_task));

	/*struct secure_task*/
		/*struct secure_task*/
	DEFINE(SECURE_TASK_SIZE, sizeof(struct secure_task));
	DEFINE(SECURE_TASK_TASK_STATE,
		offsetof(struct secure_task, task_state));

	/*optee_msg.h*/
	DEFINE(OPTEE_MSG_ARG, sizeof(struct optee_msg_arg));
	DEFINE(OPTEE_MSG_PARAM, sizeof(struct optee_msg_param));
	DEFINE(OPTEE_MSG_PARAM_VALUE, sizeof(struct optee_msg_param_value));
